...
fgarray = np.array(set_number_of_points(streamlines_64bit, 100))
mdf = dts.bundles_distances_mdf(A, A)
print(np.sum(abs(mdf-mdf.T)))
...
def test_bundles_distances_mdf(verbose=False):
    ...
    dist1 = pf.bundles_distances_mdf([xyz1A,xyz2A], [xyz2A,xyz1A])

    
    print(dist1)
    dist2 = pf.bundles_distances_mdf([xyz2A,xyz1A], [xyz1A,xyz2A])
    print(dist2)
    assert_equal(dist[0, 0], 0)
    assert_equal(dist[1, 1], 0)
    assert_equal(dist[1, 0], dist[0, 1])
